...
A robust, PyTorch-based Deep Learning classification pipeline that identifies strong gravitational lenses in astronomical multi-filter imaging data. 
...
Maintain the dataset within the project's root folder.

### 1. Train the Network
Executing the main script kicks off a fully featured 15-epoch training loop utilizing the `Adam` optimizer (LR=1e-4). Data augmentation is applied to broaden the sample generalizability.
